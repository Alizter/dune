...
We need to manually compile the prelude.
  ...
  $ coqc -boot -noinit -R lib/coq/theories/ Coq lib/coq/theories/Init/Prelude.v
...
Next we uninstall B from the stanard location, user-contrib, and install it
somewhere else.
...
build.
  ...
  $ dune build --root A
  Entering directory 'A'
  Error: The Coq theory name "B" corresponds to both:
  - $TESTCASE_ROOT/lib/coq/user-contrib/B
  - $TESTCASE_ROOT/another-place/lib/coq/user-contrib/B
  Coq theory names must correspond to a unique directory.
  Leaving directory 'A'
  [1]
