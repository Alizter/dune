...
immediately in the (depends ...) and (depopts ...) fields. Transitive and
unrelated package binaries are not implicitly exposed.
...
The directly declared provider and its build prerequisite were built:
  ...
  "_build/default/.lockfile/pkg/direct/.opam/direct/target"
  "_build/default/.lockfile/pkg/transitive/.opam/transitive/target"

Only its bin directory is added to $PATH:

  $ env_added "$(cat _build/default/path-output)" "$PATH" | censor
  $PWD/_build/default/.lockfile/pkg/direct/.opam/direct/target/bin
