  ...
  | true ->
    ...
    (match Lock_dir.read_disk lock_dir_full_path with
     ...
     | Ok lockfile ->
       ...
       in
       let pkgs =
         Lock_dir.Packages.pkgs_on_platform_by_name lockfile.packages ~platform
       in
       (match lockfile.ocaml with
        | None -> Memo.return None
        | Some (_loc, pkg_name) -> Memo.return (Package_name.Map.find pkgs pkg_name)))
;;
  ...
  | None ->
    ...
    (match sys_version with
     | Some version ->
       ...
       let constraint_ = Some (Package_constraint.Uop (Eq, String_literal version)) in
...
let any_version_locked package_name =
  ...
  if not (Fpath.exists (Path.to_string package_path))
  then false
  else (
    ...
    | Ok entries ->
      ...
      List.exists entries ~f:(fun entry ->
        ...
        Fpath.is_directory (Path.to_string entry_path)
        && Fpath.exists (Path.to_string lock_dune)))
;;
...
let lock_tool_if_needed package_name =
  if any_version_locked package_name then Memo.return () else lock_tool package_name
;;
