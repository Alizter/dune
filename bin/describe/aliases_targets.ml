      ...
      | true ->
        ...
        (match kind with
         | Target.File ->
           ...
           if (not all) && exists then None else Some (Path.Build.basename path)
         | Directory ->
           Action_builder.return (Some (Path.Build.basename path ^ Filename.dir_sep))))
  ;;
  ...
  let term =
    ...
    and+ all =
      Arg.(
        ...
        & info
            [ "all" ]
            ~doc:"Print all targets, including those that exist in the source tree.")
    in
