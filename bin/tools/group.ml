...
(* Generic commands for arbitrary packages *)
module Run = struct
...
module Add = struct
  let doc =
    "Add opam packages as tools. Use package.version syntax for specific versions."
  ;;
...
module Remove = struct
  let doc =
    "Remove a locked tool (all versions or specific version with package.version)."
  ;;
...
let group =
  Cmd.group
    info
    ~default:Tools_common.generic_list_term
