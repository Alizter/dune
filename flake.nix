{
  ...
  outputs = { self, flake-utils, opam-nix, nixpkgs, ocamllsp, opam-repository
    , melange }@inputs:
    let package = "dune";
    in flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        ocamlformat = let
          ocamlformat_version = let
            ...
            version_line = lists.findFirst ocamlformat_version_pred "not_found"
              ocamlformat_config;
            version = strings.removePrefix prefix version_line;
          in builtins.replaceStrings [ "." ] [ "_" ] version;
        ...
        testBuildInputs = with pkgs;
          [ file mercurial ] ++ lib.optionals stdenv.isLinux [ strace ];
        testNativeBuildInputs = with pkgs; [
          nodejs-slim
          pkg-config
          opam
          ocamlformat
      ...
      in {
        ...
        in {
          ...
          slim-melange = mkSlim {
            extraBuildInputs =
              [ pkgs.ocamlPackages.melange pkgs.ocamlPackages.mel ];
          };
          ...
          default = pkgs.mkShell {
            ...
            ] ++ nixpkgs.lib.attrsets.attrVals (builtins.attrNames devPackages)
              scope;
