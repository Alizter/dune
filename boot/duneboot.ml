...
module Library = struct
  ...
  module Wrapper = struct
    ...
    let mangle_filename t ({ file; kind; path = _ } : source) =
    ...
    (* Generate alias modules for qualified libraries *)
    let generate_qualified_wrapper ~build_dir ~lib_name modules =
      (* Build a trie-like structure from the qualified module names *)
      let module_trie = Hashtbl.create 10 in
      let add_to_trie qualified_name =
        let parts = String.split_on_char ~sep:'.' qualified_name in
        let rec add_parts current_trie = function
          | [] -> ()
          | [ last ] ->
            (* This is a leaf module *)
            Hashtbl.replace current_trie last `Leaf
          | part :: rest ->
            (* This is a directory level *)
            let next_trie =
              match Hashtbl.find_opt current_trie part with
              | Some (`Map t) -> t
              | Some `Leaf ->
                (* Convert leaf to map *)
                let t = Hashtbl.create 5 in
                Hashtbl.replace current_trie part (`Map t);
                t
              | None ->
                let t = Hashtbl.create 5 in
                Hashtbl.add current_trie part (`Map t);
                t
            in
            add_parts next_trie rest
        in
        add_parts module_trie parts
      in
      (* Add all modules to the trie *)
      String.Set.iter add_to_trie modules;
      (* Generate alias modules for each level *)
      let generated_files = ref [] in
      let rec generate_level current_trie current_path =
        Hashtbl.iter
          (fun name node ->
             match node with
             | `Leaf -> ()
             | `Map sub_trie ->
               let level_path = current_path @ [ name ] in
               let alias_file = String.concat ~sep:"__" level_path ^ ".ml" in
               let full_path = build_dir ^/ alias_file in
               (* Generate the alias module content *)
               Io.with_file_out full_path ~f:(fun oc ->
                 fprintf oc "(* generated by dune *)\n\n";
                 (* Add module declarations for all modules at this level *)
                 Hashtbl.iter
                   (fun sub_name sub_node ->
                      match sub_node with
                      | `Leaf ->
                        (* This is a module at this level *)
                        let qualified_name =
                          String.concat ~sep:"." level_path ^ "." ^ sub_name
                        in
                        let mangled_name =
                          String.concat ~sep:"__" level_path ^ "__" ^ sub_name
                        in
                        fprintf oc "(** @canonical %s *)\n" qualified_name;
                        fprintf oc "module %s = %s\n" sub_name mangled_name
                      | `Map _ ->
                        (* This is a subdirectory, create an alias to it *)
                        let qualified_name =
                          String.concat ~sep:"." level_path ^ "." ^ sub_name
                        in
                        let mangled_name =
                        ...
                        fprintf oc "module %s = %s\n" sub_name mangled_name)
                   sub_trie);
               generated_files := alias_file :: !generated_files;
               (* Recursively generate sublevels *)
               generate_level sub_trie level_path)
          current_trie
      in
      generate_level module_trie [];
      !generated_files
    ;;
  end

  (* Collect source files *)
  let scan ~dir ~scan_subdirs : source list =
      ...
      | file :: files ->
        let acc =
          ...
          then if scan_subdirs then loop (Io.readdir file) acc else acc
            ...
            | Some kind -> { file; kind; path = None } :: acc
            | None -> acc)
        in
        loop files acc
    in
  ...
  (* Collect source files with qualified module names *)
  let scan_qualified ~dir : source list =
    (* For qualified libraries, we want to scan from the library root directory
       and build paths relative to that directory, not including the full source path *)
    (* Create a readdir function that returns relative paths *)
    let readdir_relative base_dir =
      Sys.readdir base_dir
      |> Array.to_list
      |> List.map ~f:(fun entry -> entry)
      |> List.sort ~cmp:String.compare
    in
    let rec scan_from_dir current_dir current_path =
      let files = readdir_relative current_dir in
      List.fold_left files ~init:[] ~f:(fun acc file ->
        let file_path = current_dir ^/ file in
        if Sys.is_directory file_path
        then (
          let dir_name = file in
          let new_path = current_path @ [ dir_name ] in
          scan_from_dir file_path new_path @ acc)
        else (
          match File_kind.analyse file_path with
          | Some kind -> { file = file_path; kind; path = Some current_path } :: acc
          | None -> acc))
    in
    (* Start scanning from the library root directory *)
    (* The current_path will be relative to the library root, not the project root *)
    scan_from_dir dir []
  ;;
  ...
  let process_source_file wrapper ~header ({ file = fn; kind; path = _ } as source) =
  ...
  let process
    ...
    let scan_subdirs =
      ...
      | Qualified -> true
    in
    let files, is_qualified =
      match include_subdirs with
      | No | Unqualified -> scan ~dir ~scan_subdirs, false
      | Qualified -> scan_qualified ~dir, true
    in
    let modules =
      let modules =
        if is_qualified
        then
          List.fold_left
            ...
            ~f:(fun acc (qualified_source : source) ->
              match (qualified_source.kind : File_kind.t) with
              | Asm _ | Header | C _ -> acc
              | Ml | Mli | Mll | Mly ->
                let fn = qualified_source.file in
                let base_name =
                  (* Extract just the filename, not the full path *)
                  let filename = Filename.basename fn in
                  String.sub filename ~pos:0 ~len:(String.index filename '.')
                  |> String.capitalize_ascii
                in
                let qualified_name =
                  match qualified_source.path with
                  | None -> base_name
                  | Some path ->
                    (* For qualified modules, we want just the directory structure
                         relative to the library root, not the full source path *)
                    (* The path should already be relative to the library root *)
                    let capitalized_parts = List.map path ~f:String.capitalize_ascii in
                    String.concat ~sep:"." (capitalized_parts @ [ base_name ])
                in
                String.Set.add qualified_name acc)
        else
          List.fold_left
            files
            ~init:String.Set.empty
            ~f:(fun acc { file = fn; kind; path = _ } ->
    ...
    let header =
      Wrapper.header
        (Option.map (fun (m : Wrapper.t) -> m.alias_module) wrapper |> Option.to_list)
    in
      ...
      Fiber.fork_and_join
        (fun () ->
           let files_to_process =
             if is_qualified
             then List.map files ~f:(fun src -> { src with path = None })
             else files
           in
           Fiber.parallel_map files_to_process ~f:(process_source_file wrapper ~header))
        (fun () ->
           match build_info_module with
    ...
    let alias_file =
      if is_qualified
      then (
        let _generated_files =
          Wrapper.generate_qualified_wrapper
            ~build_dir
            ~lib_name:(Option.value ~default:"testlib" namespace)
            modules
        in
        (* For qualified libraries, we don't need a single wrapper file, 
           but we need to track the generated alias files *)
        None)
      else Wrapper.generate_wrapper wrapper modules
    in
