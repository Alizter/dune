...
let get_impl t dir =
  ...
  let package_dependencies = expander >>= Expander.package_dependencies in
  let package_env =
    let* packages = visible_packages in
    let* opam_packages = package_dependencies in
    let* context = Context.host t.context in
    let+ lock = Pkg_rules.bin_path_env ~packages (Context.name context)
    and+ opam = Opam_rules.exported_env (Context.name context) ~packages:opam_packages in
  ...
  let package_binaries =
    Memo.lazy_ ~name:"package-binaries" (fun () ->
      ...
      let* opam_packages = package_dependencies in
      let* context = Context.host t.context in
      let+ lock = Pkg_rules.package_binaries ~packages (Context.name context)
      and+ opam = Opam_rules.binaries (Context.name context) ~packages:opam_packages in
