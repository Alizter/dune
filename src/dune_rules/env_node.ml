...
type t =
  ...
  ; env_without_local_bins : Env.t Memo.Lazy.t
...
let env_without_local_bins t = Memo.Lazy.force t.env_without_local_bins
...
let make
  ...
  (* [local_bin_dirs] go ahead of the lock directory, so that a binary bound by
     ...
     where [local_bins] is consulted before the lock directory. Actions apply
     them separately, after merging in their own environment. *)
  let external_env =
    Memo.lazy_ ~name:"external-env" (fun () ->
      let+ env = Memo.Lazy.force env_without_local_bins
