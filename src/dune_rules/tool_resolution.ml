...
let find_legacy_dev_tool package_name = Dune_pkg.Dev_tool.of_package_name_opt package_name
...
let check_new_style_lock_dir ~package_name =
  ...
  | Some version ->
    ...
    Memo.return
      (Some
         ( { package = package_name; version; install_cookie; executable_hint = None; env }
         , From_locked_version ))
  | None ->
          ...
          then (
            ...
            let install_cookie =
              Path.Build.L.relative
                (Pkg_dev_tool.universe_install_path dev_tool)
                [ "target"; "cookie" ]
            in
...
let resolve_for_formatting ~package_name = resolve_opt ~package_name

(** Resolve a tool with a specific required version.
    Returns an error with a helpful message if the version isn't locked. *)
let resolve_version ~package_name ~required_version ~config_file ~executable_hint =
  let* versions = Tool_lock.get_locked_versions package_name in
  (* Check if the required version is locked *)
  match
    List.find versions ~f:(fun (v, _) -> Package_version.equal v required_version)
  with
  | Some (version, _path) ->
    ...
    let env = Tool_build.tool_env package_name ~version in
    Memo.return
      (Ok { package = package_name; version; install_cookie; executable_hint; env })
  | None ->
...
(** Resolve ocamlformat for a specific source directory.
    ...
    If no version is specified, falls back to system PATH (returns None). *)
let resolve_ocamlformat_for_dir ~dir =
  let package_name = Package.Name.of_string "ocamlformat" in
  let executable_hint = Some "ocamlformat" in
  ...
  | Some required_version ->
    (* Specific version required - try locked version, fall back to PATH *)
    let config_file =
    ...
    in
    let+ result =
      resolve_version ~package_name ~required_version ~config_file ~executable_hint
    in
    (match result with
     | Ok resolved -> Ok (Some (resolved, From_locked_version))
     | Error _msg ->
       (* Required version not locked - fall back to system PATH.
          If system PATH also fails, the user will get a "command not found" error. *)
       Ok None)
;;
