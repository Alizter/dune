...
module Context = struct
  ...
  let coq_flags ~dir ~stanza_flags ~sctx t =
    let open Action_builder.O in
    let* standard = Action_builder.of_memo @@ Super_context.coq ~dir sctx in

    Expander.expand_and_eval_set t.expander stanza_flags ~standard
  ...
  let coqdoc_file_flags ~dir cctx =
    let file_flags =
      [ theories_flags cctx
      ; A "-R"
      ; Path (Path.build dir)
...
let coqc_rule (cctx : _ Context.t) ~dir ~coq_flags ~file_flags ~coqc ~coqc_dir coq_module
    =
  let source = Coq_module.source coq_module in
  let file_flags =
...
let setup_coqc_rule ~loc ~dir ~sctx (cctx : _ Context.t) ~file_targets
    ~stanza_flags coq_module =
  let open Action_builder.With_targets.O in
  (* Process coqdep and generate rules *)
  let deps_of = deps_of ~dir ~boot_type:cctx.boot_type coq_module in
  let file_flags = Context.coqc_file_flags ~dir cctx in
  let* coqc = coqc ~loc ~dir ~sctx in
  let coq_flags = Context.coq_flags ~dir ~stanza_flags ~sctx cctx in
...
let setup_rule ~loc ~sctx ~dir ~source_rule ~file_targets ~stanza_flags cctx m =
...
let setup_coqdoc_rules ~sctx ~dir ~cctx (s : Theory.t) coq_modules =
  ...
  let rule =
    let file_flags = Context.coqdoc_file_flags ~dir cctx in
    fun mode ->
      let* () =
        let* coqdoc = coqdoc ~loc ~dir ~sctx in
        coqdoc_rule ~dir ~sctx ~mode ~theories_deps:cctx.theories_deps ~name
...
let coqtop_args_theory ~sctx ~dir ~dir_contents (s : Theory.t) coq_module =
  ...
  ( (let open Action_builder.O in
    let+ coq_flags =
      Context.coq_flags ~dir ~stanza_flags:s.buildable.flags ~sctx cctx
    in
    Command.Args.As coq_flags
    :: Command.Args.S [ Context.coqc_native_flags cctx ]
    :: Context.coqc_file_flags ~dir cctx)
  , boot_type )
...
let setup_extraction_rules ~sctx ~dir ~dir_contents (s : Extraction.t) =
  ...
  setup_rule cctx ~dir ~sctx ~loc:s.buildable.loc ~file_targets:ml_targets
    ~source_rule ~stanza_flags:s.buildable.flags coq_module
