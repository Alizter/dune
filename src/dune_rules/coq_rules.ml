    ...
    let args =
      ...
      let* native_includes =
        ...
        Resolve.Memo.map ~f:(fun lib ->
            let info = Lib.info lib in
            let obj_dir =
              Obj_dir.public_cmi_ocaml_dir (Lib_info.obj_dir info)
            in
...
let setup_coqdep_rule ~sctx ~dir ~loc ~theories_deps ~wrapper_name ~use_stdlib
  ...
  let file_flags =
    [ Command.Args.S
        (coqc_file_flags coq_module ~dir ~theories_deps ~wrapper_name
           ~use_stdlib ~ml_flags)
    ; As [ "-dyndep"; "opt" ]
...
let setup_coqc_rule ~loc ~dir ~sctx ~file_targets ~stanza_flags ~theories_deps
    ~mode ~wrapper_name ~use_stdlib ~ml_flags ~theory_dirs coq_module =
  ...
  in
  let file_flags =
    coqc_file_flags ~dir ~theories_deps ~wrapper_name ~use_stdlib ~ml_flags
      coq_module
  in
...
let setup_theory_rules ~sctx ~dir ~dir_contents (s : Theory.t) =
  ...
  let setup_coqdep_and_coqc_rule coq_module =
    setup_coqdep_rule ~sctx ~loc ~source_rule ~dir ~theories_deps ~wrapper_name
      ~use_stdlib:s.buildable.use_stdlib ~ml_flags ~mlpack_rule coq_module
    >>> setup_coqc_rule ~file_targets:[] ~stanza_flags:s.buildable.flags ~sctx
          ~loc coq_module ~dir ~theories_deps ~mode ~wrapper_name
...
let coqtop_args_theory ~sctx ~dir ~dir_contents (s : Theory.t) coq_module =
  ...
  :: coqc_file_flags ~dir ~theories_deps ~wrapper_name
...
let setup_extraction_rules ~sctx ~dir ~dir_contents (s : Extraction.t) =
  ...
  setup_coqdep_rule ~sctx ~loc:s.buildable.loc ~source_rule ~dir ~theories_deps
    ...
    coq_module
  >>> setup_coqc_rule ~file_targets ~stanza_flags:s.buildable.flags ~sctx
        ~loc:s.buildable.loc coq_module ~dir ~theories_deps ~mode ~wrapper_name
        ~use_stdlib:s.buildable.use_stdlib ~ml_flags
