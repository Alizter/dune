...
module Stanzas_to_entries : sig
  ...
  let stanzas_to_entries sctx =
    ...
    let* packages = Dune_load.packages ctx.name in
    let+ init =
...
let packages =
  let f sctx =
    ...
    let* packages = Dune_load.packages context_name in
...
let gen_package_install_file_rules sctx (package : Package.t) =
  ...
  in
  let* () =
    let* all_packages = Dune_load.packages build_context.name in
...
let scheme_per_ctx_memo =
  Memo.create
    ...
    "install-rule-scheme"
    (fun sctx ->
       Dune_load.packages (Context.name (Super_context.context sctx))
