...
let project_dependency_view ~packages ~direct_only context =
  let* dependencies = project_deps ~packages ~direct_only context in
...
let lock_dir_binaries =
  Memo.create
    ...
    (fun (context, packages) ->
       let* view = project_dependency_view ~packages ~direct_only:false context in
       let+ { Action_expander.Artifacts_and_deps.binaries; dep_info = _ } =
...
let project_ocamlpath context =
  let+ view = project_dependency_view ~packages:None ~direct_only:false context in
...
let env_for_packages ~(packages : Package.Name.Set.t option) ~(direct_only : bool) context
  ...
  | true ->
    let+ view = project_dependency_view ~packages ~direct_only context in
    let vars =
      Pkg.build_env_of_deps view.all |> Env.Map.map ~f:Value_list_env.string_of_env_values
    in
    Env.extend Env.empty ~vars
;;
