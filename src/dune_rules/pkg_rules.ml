          ...
          | Ok contents ->
            ...
            let package_dirs =
              ...
              |> List.filter_map ~f:(fun (name, kind) ->
                match kind with
                | Unix.S_DIR when not (String.starts_with name ~prefix:".") ->
    ...
    fun package_name ~version ->
      ...
      let+ db =
        ...
        | true ->
          ...
          let pkg_digest_table = Pkg_table.union ctx_db.pkg_digest_table tool_pkg_table in
          create ~pkg_digest_table ~system_provided
...
let setup_rules ~components ~dir ctx =
  ...
  | true, [ ".tools"; tool_package_name; version_string ] ->
    ...
    (match lock_exists with
     ...
     | Some _ ->
       ...
       setup_package_rules
         db
         ~package_universe:(Tool (pkg_name, version))
         ~dir
         ~pkg_digest)
  ...
  | true, ".tools" :: _ :: _ :: _ :: _ ->
    ...
    Memo.return @@ Gen_rules.redirect_to_parent Gen_rules.Rules.empty
  | is_default, [] ->
    let sub_dirs = ".pkg" :: (if is_default then [ ".dev-tool"; ".tools" ] else []) in
    let build_dir_only_sub_dirs =
...
let tool_exe_path ~package_name ~version ~executable =
  (* Compute the paths for the tool *)
  let install_path =
    Path.Build.L.relative
      Private_context.t.build_dir
      [ Context_name.to_string Context_name.default
  ...
  in
  let cookie_path = Path.Build.L.relative install_path [ "target"; "cookie" ] in
  let exe_path = Path.Build.L.relative install_path [ "target"; "bin"; executable ] in
  (* Return an action builder that depends on both cookie and exe.
     Cookie ensures package is fully built, exe ensures it's in the sandbox. *)
  let open Action_builder.O in
  let+ () = Action_builder.path (Path.build cookie_path)
  and+ () = Action_builder.path (Path.build exe_path) in
  Path.build exe_path
;;
