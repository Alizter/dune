       ...
       then from_3_0 ~with_subst ~with_sites
         ...
         | Some (_loc, dir) ->
           from_3_23
             ~with_subst
             ~with_sites
             ~runtest:("@runtest/" ^ Source_path.to_string dir)))
;;
...
let build_path ~build_dir pkg =
  let opam_path =
    Source_path.to_build_dir (Package.opam_file pkg) ~workspace_build_dir:build_dir
  in
...
let generate project pkg ~template =
  ...
  let generated_fields =
    ...
    |> filter_fields
    |> Opam_file.Create.of_bindings ~file:(Source_path.to_path opam_fname)
  in
  sprintf
...
let add_alias_rule (ctx : Build_context.t) ~profile ~project ~pkg =
  ...
  let dir =
    Source_path.to_build_dir (Dune_project.root project) ~workspace_build_dir:build_dir
  in
  let source_opam_path = Package.opam_file pkg |> Source_path.to_path in
  let opam_path =
    Source_path.to_build_dir (Package.opam_file pkg) ~workspace_build_dir:build_dir
  in
...
let gen_rules sctx ~dir ~nearest_src_dir ~src_dir =
  ...
  | Some nearest_src_dir ->
    ...
    let project_root = Source_path.as_workspace project_root |> Option.value_exn in
       ...
       then None
         ...
         let rules =
           Rules.collect_unit (fun () ->
             let* sctx = sctx
             and* project = Dune_load.find_project ~dir in
