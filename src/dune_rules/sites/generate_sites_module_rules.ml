...
let sourceroot_code buf =
  pr buf "let sourceroot = %s.sourceroot %a" helpers encode (Configpath Sourceroot)
;;

let relocatable_code buf = pr buf "let relocatable = Lazy.force %s.relocatable" helpers
...
let plugins_code packages buf pkg sites =
  ...
  List.iter sites ~f:(fun (loc, ssite) ->
    ...
    pr
      buf
      "    module %s : %s.S = %s.Make(struct let paths = Sites.%s end)"
...
let setup_rules sctx ~dir (def : Generate_sites_module_stanza.t) =
  let impl =
    ...
    if not (Package.Name.Map.is_empty plugins)
    then (
