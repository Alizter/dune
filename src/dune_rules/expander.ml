...
let source_tree_dir t = t.source_tree_dir
...
let visible_packages t =
  ...
  if lock_dir_active then package_dependencies t else Memo.return None
;;
...
let expand_pkg_macro ~loc ({ context; _ } as t) macro_invocation =
  ...
  let+ path =
    ...
    | Some (Opam _) ->
      let* visible = Action_builder.of_memo (package_dependencies t) in
      (match visible with
