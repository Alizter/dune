...
module Internal = struct
      ...
      ~f:(fun { sandbox; action_trace; input } ->
        ...
        let* action_exec_result =
          let build_deps deps =
            ...
            let+ () =
              if Option.is_none (Sandbox.root sandbox)
              then Fiber.return ()
              else
                Sandbox.add_deps
                  sandbox
  ...
  and execute_rule_impl ~rule_kind rule =
    let { Rule.id = _; targets; mode; action = _; info } = rule in
...
module Rule_shell = struct
  ...
  let rec leading_context action ~dir ~env =
    ...
    | Setenv (var, value, action) ->
      leading_context action ~dir ~env:(Env.add env ~var:(Env.Var.of_string var) ~value)
  ...
  let with_ (rule : Rule.t) ~f =
    let { Rule.targets = original_targets; _ } = rule in
    let loc = Rule.loc rule in
         ...
         ~f:(fun { sandbox; input; _ } ->
           ...
           let body, dir, shell_env =
             leading_context
               ...
               ~env:base_env
           in
           let shell_env = Dtemp.add_to_env shell_env ~purpose in
           f
             { dir
             ; shell_env
             ; replay_env
