      ...
      (match stats.st_kind with
       | S_LNK ->
         ...
         (match dir_target_root with
          | Some root ->
            let target = Unix.readlink (Path.to_string path) in
            let resolved =
              if Filename.is_relative target
              then
                (* Resolve relative symlink against the symlink's directory *)
                Path.append_local (Path.parent_exn path) (Path.Local.of_string target)
              else Path.of_string target
            in
            if not (Path.is_descendant resolved ~of_:root)
            then Error (Symlink_escapes_target path)
            else
              (match Path.stat_exn path with
               | stats -> refresh stats ~allow_dirs path
               | exception Unix.Unix_error (ELOOP, _, _) -> Error Cyclic_symlink
