...
module Context = struct
  ...
  type local_package =
    ...
    ; filtered_formulas_by_platform :
        (OpamTypes.formula * OpamTypes.formula) Solver_env.Map.t ref
    }
  ...
  let eval_to_bool (filter : OpamTypes.filter) =
    OpamFilter.eval_to_bool ~default:false (Fun.const None) filter
  ;;
  ...
  let filtered_local_formulas t ~platform local_package =
    match Solver_env.Map.find !(local_package.filtered_formulas_by_platform) platform with
...
let reject_unreachable_packages =
  ...
  in
  fun solver_env ~dune_version ~local_packages ~pkgs_by_name ~pkgs_by_name_on_platform ->
    let roots = Package_name.Map.keys local_packages in
    let pkgs_by_version =
      merge_packages
        pkgs_by_name_on_platform
        local_packages
...
let solve_lock_dir
  ...
  let platform_overlays = deduplicate_platform_overlays platform_overlays in
         ...
         let local_packages' =
           lazy
             (Package_name.Map.map local_packages ~f:(fun local ->
                ...
                ; conflicts
                ; name = local.name
                ; filtered_formulas_by_platform = ref Solver_env.Map.empty
                }))
         in
         Context.create
           ~pinned_packages
        ...
        | Ok (solution, packages_by_platform) ->
          ...
          let* pkgs_by_name =
            ...
            Result.map package_branches ~f:(fun package_branches ->
              let package_branches =
                match Package_name.Map.of_list_map package_branches ~f:Fun.id with
                | Error (name, _branches1, _branches2) ->
                  Code_error.raise
                    "Solver selected multiple versions for the same package"
                    [ "name", Package_name.to_dyn name ]
                | Ok package_branches -> package_branches
              in
              (* Compute reachability independently for each platform. Each
                 dependency graph uses only the package branches evaluated for
                 that platform. *)
              let reachable_by_platform =
                List.fold_left
                  full_solver_envs
                  ~init:Solver_env.Map.empty
                  ~f:(fun acc solver_env ->
                    let pkgs_by_name_on_platform =
                      Package_name.Map.filter_map package_branches ~f:(fun branches ->
                        List.find_map branches ~f:(fun (branch_solver_env, pkg) ->
                          Option.some_if
                            (Solver_env.equal branch_solver_env solver_env)
                            pkg))
                    in
                    let reachable =
                      reject_unreachable_packages
                        ...
                        ~local_packages
                        ~pkgs_by_name:pkgs_by_name_on_platform
                        ~pkgs_by_name_on_platform
                    in
                    Solver_env.Map.set acc solver_env reachable)
              in
              (* Drop unreachable platform branches before merging package
                 ...
                 on another platform. *)
              Package_name.Map.to_list_map package_branches ~f:(fun name branches ->
                let reachable_branches =
                  List.filter_map branches ~f:(fun (solver_env, pkg) ->
                    match Solver_env.Map.find reachable_by_platform solver_env with
                    | Some reachable ->
                      Option.some_if (Package_name.Set.mem reachable name) pkg
                    | None ->
                      Code_error.raise
                        "Reachability was not computed for solver environment"
                        [ "solver_env", Solver_env.to_dyn solver_env ])
                in
                match reachable_branches with
                | [] -> None
                | first :: rest ->
                  let pkg =
                    List.fold_left rest ~init:first ~f:Lock_dir.Pkg.merge_conditionals
                  in
                  Some (name, pkg))
