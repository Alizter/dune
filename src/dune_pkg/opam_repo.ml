...
let all_package_versions t opam_package_name =
  ...
  if Path.exists version_dir_path
  then (
...
let load_opam_package t opam_package =
  ...
  if not (Path.exists opam_file_path)
  then
    User_error.raise
      [ Pp.textf
          "Couldn't find package file for \"%s\". It was expected to be located in %s \
           but this file does not exist"
          (OpamPackage.to_string opam_package)
          (Path.to_string_maybe_quoted opam_file_path)
      ];
  OpamFile.OPAM.read (OpamFile.make (OpamFilename.raw (Path.to_string opam_file_path)))
;;
